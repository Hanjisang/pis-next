...
终端打印复用既有 `ReportOutputPort`、不可变PDF、打印机配置、身份二次核验、幂等命令和 `report_print_record`，没有创建第二套报告或打印事实；真实患者终端和打印硬件仍由既有设备外部依赖项承担。验证证据：`V2ReportWebTest` 覆盖临床查询、错误身份拒绝、患者核验、终端打印及撤回后不可查询；`V2ReportCenter.test.ts` 覆盖两个入口和打印请求；`report-access.spec.ts` 覆盖 1920/1366 两档产品浏览器流程。覆盖矩阵复算为 `TOTAL=742 / COMPLETE=341 / PARTIAL=119 / MISSING=202 / EXTERNAL_DEPENDENCY=80 / CONFLICT_RESOLVED_BY_V2=0`。
